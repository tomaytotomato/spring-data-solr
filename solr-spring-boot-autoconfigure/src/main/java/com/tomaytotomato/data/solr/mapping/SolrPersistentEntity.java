 ...
 *
 * <p>Wraps a domain class annotated with {@link SolrEntity} and exposes the Solr collection
...
public class SolrPersistentEntity<T> extends BasicPersistentEntity<T, SolrPersistentProperty> {
   ...
   * {@link Environment}.
   *
   * <p>The Solr collection name is resolved from the {@link SolrEntity#collection()} attribute.
  ...
  public SolrPersistentEntity(TypeInformation<T> typeInformation, Environment environment) {
    ...
    var annotation = typeInformation.getType().getAnnotation(SolrEntity.class);
