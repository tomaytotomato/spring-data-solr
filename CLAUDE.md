...
Four-module Maven project. Package root: `com.tomaytotomato.data.solr`.
...
- **`solr-spring-boot-sample`** — demo Spring Boot app with Docker Compose support for local Solr.
  The `curated-books.json` dataset it loads is **generated by the curator module** — do not edit it
  by hand
- **`solr-spring-boot-data-curator`** — standalone tool that reads the Kaggle 7k Books CSV and
  produces `curated-books.json`. Run `BookCurator.main()` from the repo root to regenerate the
  dataset. To change the dataset size, adjust `TARGET_TOTAL` in `BookCurator.java`
...
`docs/DEVLOG.md` is the single dev log — Days 1–5, from 2026-05-12 onwards.
